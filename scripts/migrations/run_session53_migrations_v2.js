/**
 * Run Session 53 Migrations v2
 * ============================
 *
 * Uses direct subqueries instead of functions in materialized views
 * to avoid schema search path issues.
 *
 * Usage: node scripts/migrations/run_session53_migrations_v2.js
 */
...
async function runQuery(client, description, sql) {
  try {
    ...
    console.log(`✅ ${description}` + (result.rowCount !== null ? ` (${result.rowCount} rows)` : ''));
    return true;
  } catch (e) {
...
async function main() {
  ...
  console.log("🚀 RUNNING SESSION 53 MIGRATIONS v2");
  console.log("=".repeat(70));
  ...
  try {
    ...
    await runQuery(client, "Mark other seasons as not current",
      `UPDATE seasons SET is_current = false WHERE (start_date != '2025-08-01' OR end_date != '2026-07-31') AND is_current = true`
    );

    // Create the function with explicit schema
    await runQuery(client, "Create get_current_season_year function",
      `CREATE OR REPLACE FUNCTION public.get_current_season_year()
       RETURNS INTEGER AS $$
       DECLARE
         season_year INTEGER;
       BEGIN
         SELECT year INTO season_year FROM public.seasons WHERE is_current = true LIMIT 1;
    ...
    // Test the function
    const funcTest = await client.query("SELECT public.get_current_season_year() as year");
    console.log(`✅ Function test: get_current_season_year() = ${funcTest.rows[0].year}`);
    ...
    // Use subquery instead of function for view
    await runQuery(client, "Create teams_v2_live view",
      `CREATE OR REPLACE VIEW teams_v2_live AS
       SELECT
         t.*,
         CASE WHEN t.birth_year IS NOT NULL
           THEN 'U' || ((SELECT year FROM public.seasons WHERE is_current = true LIMIT 1) - t.birth_year)::TEXT
           ELSE NULL
         END AS age_group_computed,
         (SELECT year FROM public.seasons WHERE is_current = true LIMIT 1) AS season_year
       FROM teams_v2 t`
    );
    ...
    // ============================================================
    console.log("\n" + "=".repeat(70));
    console.log("MIGRATION 023: Update materialized views");
    console.log("=".repeat(70));

    // Get current season year to use in queries
    const seasonResult = await client.query("SELECT year FROM public.seasons WHERE is_current = true LIMIT 1");
    const SEASON_YEAR = seasonResult.rows[0]?.year || 2026;
    console.log(`📅 Using season year: ${SEASON_YEAR}`);

    // app_rankings - use hardcoded season year (will be recomputed on refresh)
    await runQuery(client, "Drop app_rankings",
      `DROP MATERIALIZED VIEW IF EXISTS app_rankings CASCADE`
    );

    await runQuery(client, "Create app_rankings",
      `CREATE MATERIALIZED VIEW app_rankings AS
       SELECT
         ...
         CASE WHEN t.birth_year IS NOT NULL
           THEN 'U' || (${SEASON_YEAR} - t.birth_year)::TEXT
           ELSE NULL
         END as age_group,
    ...
    await runQuery(client, "Create app_rankings indexes",
      `CREATE UNIQUE INDEX idx_app_rankings_id ON app_rankings (id)`
    );

    await runQuery(client, "Create app_rankings filter index",
    ...
    await runQuery(client, "Create app_matches_feed",
      `CREATE MATERIALIZED VIEW app_matches_feed AS
       SELECT
         ...
         CASE WHEN ht.birth_year IS NOT NULL
           THEN 'U' || (${SEASON_YEAR} - ht.birth_year)::TEXT
           ELSE NULL
         END as age_group,
         ht.state
       FROM matches_v2 m
    ...
    await runQuery(client, "Create app_matches_feed indexes",
    ...
    await runQuery(client, "Create app_upcoming_schedule",
    ...
    await runQuery(client, "Create app_upcoming_schedule indexes",
    ...
    const yearCheck = await client.query("SELECT public.get_current_season_year() as year");
    ...
    try {
      ...
      console.log(`❌ teams_v2_live: ${e.message}`);
    }

    const seasonCheck = await client.query("SELECT year, is_current FROM public.seasons WHERE is_current = true");
    ...
    console.log(`✅ app_rankings: ${rankingsCheck.rows[0].cnt} rows`);

    const matchesCheck = await client.query("SELECT COUNT(*) as cnt FROM app_matches_feed");
    console.log(`✅ app_matches_feed: ${matchesCheck.rows[0].cnt} rows`);
