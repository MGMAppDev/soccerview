...
> **Version 7.0** | Last Updated: February 9, 2026 | Session 94 Part 2 Complete
>
...
│   • NO constraints       • UNIQUE source_match_key• NO constraints             │
...
│                                                                                 │
│   staging_standings (Session 92)                                                │
│   ──────────────────────────────                                                │
│   • All fields TEXT/INTEGER        • league_source_id, team_name               │
│   • NO foreign keys                • W-L-D, GF, GA, Pts, Position, RC          │
│   • Source platform + snapshot     • age_group, gender, division               │
│   • processed flag                 • extra_data JSONB                          │
│                                                                                 │
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   teams_v2 (145,356)     matches_v2 (402,948*)   clubs (124,650)                │
│   ──────────────────     ──────────────────      ─────────────                  │
...
│                                                                                 │
│   leagues (280)          tournaments (1,750)     venues, schedules             │
│   ─────────────          ──────────────────      ──────────────────            │
│   • source_event_id      • source_event_id       • Location data               │
│   • Season tracking      • Date range            • Field names                 │
│                                                                                 │
│   league_standings (1,208) (Session 92)                                        │
│   ─────────────────────────────────────                                        │
│   • UUID pk, FKs to leagues + teams_v2                                         │
│   • W-L-D, GF, GA, GD (generated), Pts, Position                              │
│   • UNIQUE (league_id, team_id, division)                                      │
│   • Source: staging_standings → processStandings.cjs                           │
│                                                                                 │
│   * 402,948 active + ~5,297 soft-deleted (Sessions 86-94: soft-delete arch)    │
│                                                                                 │
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   app_rankings (145,356)           app_team_profile (145,356)                   │
│   ──────────────────────           ──────────────────────────                   │
...
│                                                                                 │
│   app_matches_feed (402,948)       app_league_standings (15,370)               │
│   ──────────────────────────       ─────────────────────────────               │
│   • home_team JSONB embedded       • HYBRID: scraped UNION computed (Sess 92) │
│   • away_team JSONB embedded       • Part 1: league_standings (authoritative) │
│   • event JSONB embedded           • Part 2: matches_v2 fallback (legacy)     │
│   • venue JSONB embedded           • Form, Position, Goal difference          │
│                                                                                 │
...
## Dual-System Architecture: Match Pipeline vs Standings Absorption

### Principle 36: Two Pipelines, Two Resolvers, One Shared Team Registry

SoccerView operates TWO independent data systems. This is by design.

```
┌─────────────────────────────────────────────────────────────────────────┐
│                SYSTEM 1: MATCH PIPELINE (Rankings/ELO)                   │
│                                                                          │
│  Scrapers → staging_games → DQE/fastProcess → matches_v2 → ELO          │
│                                                                          │
│  Heavy 3-tier entity resolution:                                         │
│    Tier 1: source_entity_map (deterministic)                             │
│    Tier 2: Canonical name + NULL-tolerant metadata                       │
│    Tier 3: pg_trgm fuzzy matching (>0.5 similarity)                      │
│                                                                          │
│  Purpose: Rankings, ELO, Teams tab, Matches tab, Team Details            │
│  Scripts: dataQualityEngine.js, fastProcessStaging.cjs                   │
│  Output:  matches_v2, teams_v2 (ELO ratings), rank_history_v2            │
│                                                                          │
│  UNCHANGED — This system is NOT affected by standings work.              │
└─────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────┐
│           SYSTEM 2: STANDINGS ABSORPTION (League Standings)               │
│                                                                          │
│  Adapters → staging_standings → processStandings → league_standings       │
│                                                                          │
│  Lightweight team resolution (Session 92 QC):                            │
│    Step 1: source_entity_map + METADATA VERIFICATION                     │
│            If metadata incompatible → find enriched alternative           │
│    Step 2: Exact name + birth_year + gender match                        │
│    Step 3: Create new team (trust the league — NO fuzzy matching)         │
│                                                                          │
│  Purpose: League Standings page ONLY                                     │
│  Script:  processStandings.cjs                                           │
│  Output:  league_standings → app_league_standings (PART 1)               │
│                                                                          │
│  WHY SEPARATE: League data is authoritative ("pure-ish"). The league     │
│  publishes exactly which teams are in which division with W-L-T.         │
│  Fuzzy matching causes false positives. Lightweight absorption is safer. │
└─────────────────────────────────────────────────────────────────────────┘

Shared resource: teams_v2 (standings needs FKs)
  - Standings READS teams_v2 for resolution
  - Standings ENRICHES NULL fields (birth_year, gender) — never overwrites
  - Standings CREATES new teams when exact match fails
  - Zero impact on match pipeline's ELO, rankings, or match history
```

### Why Two Resolvers?

| Aspect | Match Pipeline (Heavy) | Standings Absorption (Light) |
|--------|----------------------|----------------------------|
| **Data quality** | Messy (cross-source, inconsistent) | Authoritative (league-published) |
| **Source count** | Multiple (GotSport, HTG, Heartland) | ONE per league |
| **Team IDs** | Often missing | Provided by source |
| **Age/Gender** | Inferred from names | Explicitly published |
| **Fuzzy matching** | YES (cross-source dedup needed) | NO (single authority) |
| **Risk of wrong match** | Low (fuzzy is last resort) | HIGH (fuzzy = false positives) |
| **On failure** | Skip (match still importable later) | Create new team (safer) |
...
- Mixing the two pipelines in the same script

---

## League Standings Pipeline (Session 92 + QC Fix)

### Hybrid Passthrough Architecture

The league IS the authority on its own standings. Instead of recomputing from match data, we scrape authoritative standings and display as-is.

```
  Adapter standings config          scrapeStandings.js (universal engine)
  (heartland.js, etc.)        ───►  Writes to staging_standings
                                           │
                                           ▼
                                    processStandings.cjs
                                    LIGHTWEIGHT resolver (Session 92 QC):
                                    Step 1: source_entity_map + metadata verify
                                    Step 2: Exact name + birth_year + gender
                                    Step 3: Create new team (no fuzzy)
                                           │
                                           ▼
                                    ...
                                    UNIQUE (league_id, team_id, division)
                                           │
                                           ▼
                              app_league_standings (hybrid UNION ALL)
                              ┌─────────────────────────────────────┐
                              │ PART 1: Scraped standings            │
                              │ (leagues WITH league_standings data) │
                              ├─────────────────────────────────────┤
                              │ PART 2: Computed fallback            │
                              │ (leagues WITHOUT scraped data)       │
                              │ Same CTE as before = zero behavior   │
                              │ change for non-scraped leagues       │
                              └─────────────────────────────────────┘
```

### Lightweight Team Resolution (processStandings.cjs)

```javascript
// Step 1: source_entity_map + METADATA VERIFICATION
//   → Found + metadata compatible → enrich NULLs → use it
//   → Found + metadata INCOMPATIBLE → find enriched alternative → redirect
//   → Not found → Step 2

// Step 2: Exact name + birth_year + gender match
//   → Prefers records WITH metadata over NULL
//   → Found → register in source_entity_map → use it
//   → Not found → Step 3
...
| Script | Purpose |
|--------|---------|
| `scripts/universal/scrapeStandings.js` | Universal standings scraper engine |
| `scripts/maintenance/processStandings.cjs` | staging_standings → league_standings (lightweight resolver) |
| `scripts/adapters/heartland.js` | Heartland standings adapter (archives + live CGI) |
| `scripts/migrations/094_league_standings_passthrough.sql` | Tables + hybrid view |
| `scripts/_debug/verify_standings_completeness.cjs` | 1:1 verification per division |
| `scripts/_debug/diagnose_standings_gaps.cjs` | Gap diagnostic (A/B/C/D classification) |

### Adding Standings for a New Source (Universal — 200-400 Sources)

1. Add `standings` section to adapter config (e.g., `htgsports.js`)
2. Define `staticSources[]` or `discoverSources()` function
3. Implement `scrapeSource(engine, source)` → returns standings array
4. Run: `node scripts/universal/scrapeStandings.js --adapter {source}`
5. Process: `node scripts/maintenance/processStandings.cjs`
6. Verify: `node scripts/_debug/verify_standings_completeness.cjs`

**Time to add new standings source: ~1-2 hours (adapter config only)**
**Zero custom code per source.** The lightweight resolver handles all sources universally.
...
| 2 | matches_v2 | home_team_id ≠ away_team_id | dataQualityEngine | Skip with error log |
...
Match deduplication uses soft-delete, NOT hard-delete. Session 85 hard-deleted 9,160 matches — Session 86 recovered 6,053 and established the soft-delete pattern.
...
- `app_team_profile` — match subquery
- `app_matches_feed` — main WHERE clause
- `app_league_standings` — JOIN condition + form subquery
...
Record B: date=Aug 14, home=TeamY, away=TeamX, score=3-2  ← ALLOWED by constraint
```
...
**Current counts:** 402,948 active matches + ~5,297 soft-deleted (Sessions 86-94).
...
|------|--------|----------|-------|
| **Tier 1** | Source ID → `source_entity_map` → SV UUID | 100% | O(1) lookup |
...
Scraper emits match with source_team_ids
  → Tier 1: Look up source_entity_map (deterministic, fastest)
  → Tier 2: Exact canonical_name match (NULL-tolerant birth_year)
  → Tier 3: Fuzzy match (suffix, token-based)
  → Create new team + register in source_entity_map
```

**Current counts:** ~72,000+ source entity mappings (68K+ GotSport teams + 1,244 Heartland teams + 274 leagues + 1,735 tournaments). Session 94 backfilled GotSport team IDs for Tier 1 deterministic matching.
...
BEGIN
  ...
  -- app_league_standings: NO CONCURRENTLY (hybrid UNION ALL view has no unique index)
  -- Migration 095 fix — matches refresh_views_manual.js behavior
  REFRESH MATERIALIZED VIEW app_league_standings;
END;
$$ LANGUAGE plpgsql;
```

**Note (Session 92 QC Part 2):** `app_league_standings` uses non-concurrent refresh because the hybrid UNION ALL view (migration 094) has no unique index. `CONCURRENTLY` requires a unique index and will fail without one, causing the entire `refresh_app_views()` function to roll back (all views stay stale). Migration 095 fixed this.
...
jobs:
  ...
  intake-validation:    → intakeValidator.js (reject garbage, fix malformed)

  # Phase 2: Normalize, resolve, promote to production
  data-quality-engine:  → dataQualityEngine.js → teams_v2, matches_v2, leagues, tournaments

  # Phase 2.5: Self-healing inference linkage (Session 56)
  infer-event-linkage:  → Links orphaned matches by team patterns

  # Phase 2.6: Process standings (Session 92)
  process-standings:    → staging_standings → league_standings (lightweight resolver)

  # Phase 2.7: GotSport Rankings Refresh (Session 94)
  refresh-gotsport-rankings: → restoreGotSportRanks.cjs --execute (restore official ranks after team merges)
...
| `dataQualityEngine.js` | **THE processor** - Normalize, resolve, promote (Session 79) | 1 → 2 |
| `restoreGotSportRanks.cjs` | **NIGHTLY** Restore GotSport official ranks after team merges (Session 94) | 2 |
