...
### Match Identification (source_match_key) - Session 57
...
**Constraint:** `UNIQUE (source_match_key)` - enforced on matches_v2

**Upsert Strategy:** Validation pipeline uses `ON CONFLICT (source_match_key)` for idempotent inserts.

**Note:** The `unique_match` constraint (on date + teams + score) was dropped in Session 57 as it conflicted with the upsert strategy.
