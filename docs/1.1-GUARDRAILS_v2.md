...
> 2. Treat every rule as an absolute constraint — not a suggestion.
...
- **NEVER** batch multiple UI changes together. One change → verify → next change.
- **NEVER** change component structure, styling, spacing, or layouts.
- If the UI shows wrong data, fix the **data source** — not the UI.
...
```
Staging Tables → dataQualityEngine → Normalizers → Canonical Registries → Production Tables
```
...
```sql
-- ❌ WRONG — Data is lost permanently
DELETE FROM matches_v2 WHERE id = ANY($1);

-- ✅ CORRECT — Data is preserved for recovery
UPDATE matches_v2
SET deleted_at = NOW(),
...
### Division Detection
```javascript
// ✅ CORRECT — dash is optional
/U-?\d{1,2}\b|20[01]\d/i

// ❌ WRONG — requires dash, misses "U12"
/U-\d+/
```

### Age Group Extraction
```javascript
// ✅ CORRECT — handles "U12", "U-12", "U 12"
/\bU[-\s]?(\d{1,2})\b/i

// ❌ WRONG — only matches "U12"
/u(\d{1,2})/
```

### NULL Score Preservation
```javascript
// ✅ CORRECT — preserves scheduled matches
home_score: hasValidScores ? homeScore : null

// ❌ WRONG — turns scheduled matches into 0-0 results
...
### Bulk Operations
```javascript
// ❌ WRONG — takes hours for 100K+ records
for (const team of teams) {
  await supabase.from('teams_v2').update({ elo: newElo }).eq('id', team.id);
}

// ✅ CORRECT — takes seconds for 100K+ records
...
Scrapers / Adapters     → Validation / Normalizers    → Views / App UI
```
...
1. **Does this bypass V2 architecture?** → STOP. Use the proper pipeline.
2. **Does this touch UI code (.tsx)?** → STOP. Get explicit user approval first.
3. **Is this a hack or a proper solution?** → If hack, STOP.
4. **Does this change the schema the UI expects?** → STOP. Views must match existing column names.
5. **Have I verified this against CLAUDE.md principles?** → Must be YES.
6. **Will this affect all 3 layers?** → Verify propagation through each.
7. **Am I batching multiple changes?** → STOP. One at a time.
...
| `scripts/universal/intakeValidator.js` | **NEW (Session 79)** Pre-staging validation gate | L1→L2 |
| `scripts/universal/dataQualityEngine.js` | **THE ONLY** staging→production path | L2 |
...
- ✅ `dataQualityEngine.js` - Authorized
- ✅ `recalculate_elo_v2.js` - Authorized
- ✅ `mergeTeams.js` - Authorized
- ✅ `mergeEvents.js` - Authorized
- ✅ `inferEventLinkage.js` - Authorized
- ✅ Deduplication scripts - Authorized
...
|----------|----------------------|---------|
| Core Pipeline | ✅ Yes | dataQualityEngine.js |
| Maintenance | ✅ Yes | mergeTeams.js, fixDataDisconnect.cjs |
| Scrapers | ❌ No (writes to staging) | coreScraper.js |
| Debug | ⚠️ Review before use | scripts/_debug/*.cjs |
...
### Start of Session
```
□ Read GUARDRAILS.md (this file) in full
□ Read CLAUDE.md principles section
□ Check Master Project Scope for current state and immediate next step
□ Ask user for current file versions before rewriting anything
□ Identify which layers the current task affects
□ Plan verification steps for each affected layer
```
...
□ Master Project Scope updated with progress?
□ CRITICAL_RULES SYNC: If new principles added, update .claude/hooks/CRITICAL_RULES.md
□ GIT COMMIT: All changes committed and pushed?
□ GIT STATUS: Working directory clean (except .env)?
```
...
### Anti-patterns
- ❌ Ending session with uncommitted changes
- ❌ Accumulating 10+ files without committing
- ❌ Waiting for user to ask "did you commit?"
...
3. **Fill Gaps First:** Identify and fill V1→V2 gaps
4. **Foundation Quality:** Daily scrape BUILDS ON the foundation

### Anti-patterns

- ❌ Assuming V2 is complete because it has "more records"
- ❌ Comparing only total counts, not same-date/same-team matches
- ❌ Skipping V1 migration because V2 "looks bigger"
- ❌ Starting daily scrape before foundation is solid
...
V1 rank_history had 49,729 entries MISSING from V2 for the same dates, even though V2 had more total records. **Volume ≠ completeness.**
...
- ❌ Fuzzy matching that ignores `birth_year` or `gender`
- ❌ Grouping teams by name only (missing constraining fields)
- ❌ Auto-merging without audit trail
- ❌ Skipping canonical registry lookup
- ❌ Creating entities without self-learning step
...
## 19. SEASON COMPLETENESS — ALWAYS Scrape the FULL Current Season (Session 99)

**ABSOLUTE RULE: The SoccerView season runs August 1 → July 31. Every scraping task MUST ensure coverage of BOTH the Fall half (Aug-Dec) AND the Spring half (Feb-Jun) of the current season.**

### Season Calendar

| Month | Season Phase | Scraping Action |
|-------|-------------|----------------|
| Aug-Nov | Fall season (PEAK league play) | Scrape Fall events IMMEDIATELY — this is where most matches happen |
| Dec-Jan | Winter break / indoor | Scrape winter leagues where active |
| Feb-Mar | Spring ramp-up | Discover Spring event IDs, begin Spring scraping |
| Apr-Jun | Spring season | Scrape Spring events, full coverage |
| Jul | Off-season | Plan next season |

### Critical Rules

1. **NEVER look only at "current" or "upcoming" events.** Always check: Do we have Fall data for this season?
2. **"Between seasons" means look for the OTHER half** — NOT "no data exists."
3. **`year` field = season END year** (2026 for the 2025-26 season), NOT the event's calendar year.
4. **SportsAffinity uses DIFFERENT subdomains per season** (e.g., `gs-fall25{orgcode}` for Fall 2025).
5. **GotSport uses SEPARATE event IDs** for Fall vs Spring seasons.
6. **0 matches from a scrape = investigate immediately** — check if wrong season, wrong event ID, or between-seasons.
7. **Every new state onboarding must include BOTH Fall + Spring events** from the current season.
...
```
□ Do we have Fall (Aug-Dec) data for this state?
□ Do we have Spring (Feb-Jun) data for this state?
□ Are we using the correct event IDs for BOTH seasons?
□ If a platform uses different subdomains per season, do we have ALL season subdomains?
□ If 0 matches returned, have we checked the other season half?
```

### Anti-patterns (BANNED)

- Accepting 0 matches as "between seasons" without checking Fall events
- Configuring ONLY Spring 2026 events when Fall 2025 data exists
- Forgetting that the current season started in August 2025
- Not checking whether a platform uses different IDs/subdomains for different seasons

### DATA RETENTION WARNING — Some Platforms PURGE Data Between Seasons (Session 105)
...
|----------|---------------|------------|
| GotSport | ✅ Retains all seasons | LOW |
| SportsAffinity | ✅ Retains via season subdomains | LOW |
| PlayMetrics | ✅ Retains via season URLs | LOW |
| Demosphere | ✅ Retains via season endpoints | LOW |
| SINC Sports | ✅ Retains per-season data | LOW |
| Heartland CGI | ✅ Retains dynamically | LOW |
| Squadi | ✅ Retains via REST API | LOW |
| **RI Super Liga** | **❌ PURGES between seasons** | **CRITICAL** |
| TotalGlobalSports | ✅ Retains per-event | LOW |
| MLS Next | ✅ Retains per-season | LOW |

**RI Super Liga (thesuperliga.com) — PERMANENT DATA LOSS:**
...
- **MUST scrape during active season:** Spring ~March 28 → June, Fall ~Aug → Oct.
...
2. If YES → safe to build adapter any time, backfill later
3. If NO → **MUST deploy adapter BEFORE the current season ends or data is lost forever**
...
## 18. NEVER ACCEPT "BLOCKED" — FIND THE BACK DOOR (Session 99)

**ABSOLUTE RULE: When a data source appears blocked, inaccessible, or difficult to scrape — you MUST find a way around it. No excuses. No deferring. No "we'll come back to it later."**
...
| Different data format | "Doesn't match our schema" | Write an adapter — that's literally what adapters are for |

### Techniques to Deploy (minimum 5 before deferring)

1. **Inspect Network tab** — Most SPAs fetch data via XHR/fetch. Find the API endpoint directly.
2. **Check page source** — Many sites embed JSON in `<script>` tags or `window.__INITIAL_STATE__`.
3. **Try `/api/`, `/json/`, `/data/` endpoints** — Many platforms have undocumented public APIs.
4. **Use Puppeteer stealth** — `puppeteer-extra-plugin-stealth` bypasses most bot detection.
5. **Check mobile endpoints** — Mobile apps often hit simpler, less-protected APIs.
6. **Vary User-Agent strings** — Try Googlebot, mobile browsers, curl patterns.
7. **Look for embed/widget URLs** — Many platforms have embeddable widgets with public access.
8. **Check Wayback Machine** — Historical data may still be accessible.
9. **Search for alternative data sources** — Same league data may exist on a different platform.
10. **Use WebSearch to research** — Other developers may have documented the API in forums/GitHub.

### When "Between Seasons" — NEVER Mark as Done
...
- ❌ "This site blocks scrapers" → without trying stealth/different approaches
- ❌ "Data isn't available" → without checking multiple seasons/endpoints
- ❌ "We'll come back to this" → without a specific retry plan and date
- ❌ "This platform is too complex" → that's what Puppeteer + adapters are for
- ❌ Marking a state as "GS RANKS" when league data exists somewhere online
- ❌ Accepting empty results from a scrape without investigating why
